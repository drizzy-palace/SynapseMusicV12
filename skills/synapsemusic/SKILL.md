---
name: synapsemusic
description: 使用 Synapse Music V12 API 生成音乐、编辑歌曲、remix 音乐。支持文本生成音乐、歌词生成、音频续写、音频重绘等功能。当用户提到生成音乐、创作歌曲、音乐制作、remix、续写音频时使用此技能。
allowed-tools: Read, Write, Bash
---

# Synapse Music 音乐生成技能

使用 Synapse Music V12 API 进行音乐生成和编辑。

## 输出文件

生成完成后，脚本会自动保存结果到项目根目录的 `synapsemusic_output` 文件夹（与 `.claude` 同级）：

```
project_root/
├── .claude/
│   └── skills/synapsemusic/...
├── synapsemusic_output/          # 输出目录
...
./scripts/synapsemusic.sh generate "流行音乐"

# 临时覆盖（本次 thinking=false，但 config.json 不变）
./scripts/synapsemusic.sh generate "流行音乐" --no-thinking

# 永久修改默认配置
./scripts/synapsemusic.sh config --set generation.thinking false
```
...
| `generation.thinking` | `true` | 启用 Synapse Composer 模型（高质量模式） |
...
```bash
# 查看所有配置
./scripts/synapsemusic.sh config

# 列出所有配置项及当前值
./scripts/synapsemusic.sh config --list

# 获取单个配置值
./scripts/synapsemusic.sh config --get generation.thinking

# 永久修改配置值（会写入 config.json）
./scripts/synapsemusic.sh config --set generation.thinking false
./scripts/synapsemusic.sh config --set api_url http://192.168.1.100:8001

# 重置为默认配置
./scripts/synapsemusic.sh config --reset
```
...
```bash
./scripts/synapsemusic.sh generate "流行音乐，吉他伴奏"
./scripts/synapsemusic.sh generate -c "抒情流行" -l "[Verse] 歌词内容"
```

**Simple 模式** - 用简单描述，LM 自动生成 caption 和 lyrics
```bash
./scripts/synapsemusic.sh generate -d "一首关于春天的欢快歌曲"
./scripts/synapsemusic.sh generate -d "A love song for February"
```

**其他选项**
```bash
# 临时禁用 thinking 模式（本次生效，不修改配置文件）
./scripts/synapsemusic.sh generate "电子舞曲" --no-thinking

# 临时禁用 format 模式
./scripts/synapsemusic.sh generate "古典钢琴" --no-format

# 临时指定其他参数
./scripts/synapsemusic.sh generate "爵士乐" --steps 16 --guidance 8.0

# 随机生成
./scripts/synapsemusic.sh random

# 查询任务状态（已完成的任务会自动下载音频）
./scripts/synapsemusic.sh status <job_id>

# 列出可用模型
./scripts/synapsemusic.sh models

# 检查 API 健康状态
./scripts/synapsemusic.sh health
```
...
| 脚本 | 依赖 | 平台 |
|------|------|------|
| `synapsemusic.sh` | curl | Linux/macOS/Git Bash |
...
| `lyrics` | string | "" | 歌词内容 |
| `thinking` | bool | true | 启用 Synapse Composer 模型（高质量） |
...
5. **自动下载**: 任务完成后会自动保存 JSON 结果并下载音频文件到 `synapsemusic_output/` 目录

## 参考资源
- Shell 脚本: [scripts/synapsemusic.sh](scripts/synapsemusic.sh) (Linux/macOS/Git Bash)
- 默认配置: [scripts/config.json](scripts/config.json)
- 输出目录: 项目根目录下的 `synapsemusic_output/`
